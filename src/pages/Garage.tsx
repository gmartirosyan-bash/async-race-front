...
export default function Garage() {
  ...
  return (
    <>
      ...
      <div
        className="w-300 m-auto h-15 bg-[length:60px] bg-[url('/border.png')] bg-repeat-x bg-center"
      >
      </div>
      ...
      <div>
        <ul>
          {cars.map(car => {
            const RandomCar = carSvgs[Math.floor(Math.random() * carSvgs.length)]
            return (
              <li key={car.id} style={{ color: car.color }} className="flex items-center ">
                <p className="w-30 mr-4">
                {car.name}
                </p>
                <RandomCar  className="w-70 h-35" fill={car.color} />
              </li>
            )
          })}
        </ul>
